...
int dfs( int node, vector<int> &lava, unordered_map<int,list<int>> &adj, int life, vector<int> &wt, int time ) {

    life += wt[node];
    if ( life == 0 || lava[node] <= time ) return 0;
    int ans = 0;
    for ( auto it : adj[node] ) {
        ans = max( ans , 1 + dfs(it, lava, adj, life, wt, time+1) );
    }
    return ans;
}
...
int main () {
    ...
    while ( t-- ) {
        ...
        int ans = dfs( st, lava, adj, 1, wt, 0 );
        cout << ans << endl;
    }
    return 0;
}
